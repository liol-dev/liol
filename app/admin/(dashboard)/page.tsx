...
import { fetchCategories } from "@/app/lib/categories.server";
import {
  countByCategory,
  newestFirst,
  formatPhotoDate,
...
export default async function AdminOverviewPage() {
  ...
  const [photosResult, categories] = await Promise.all([
  ...
  // Filter miscellaneous out of the donut — it's a fallback, not a display category
  const donutData = categories
    .filter((cat) => cat.slug !== "miscellaneous")
    .map((cat, i) => ({
      label: cat.label,
      value: counts[cat.slug] ?? 0,
  ...
  return (
    <>
      ...
      {photosResult.error && (
        <p className="mt-4 text-sm text-red-400">Could not load photos. Please try again.</p>
      )}

      {/* ── Stat cards ───────────────────────────────────── */}
      <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-3">
        <StatCard label="Total photos" value={String(total)} />
        <StatCard label="Categories" value={String(categories.filter(c => c.slug !== "miscellaneous").length)} />
        <StatCard
          label="Last upload"
      ...
      <div className="mt-6 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6 items-start">
        ...
        <section aria-label="Category distribution" className="rounded-xl bg-liol-text/5 p-5">
          <h2 className="text-xs text-liol-subtext">Category distribution</h2>
          ...
          <ul className="mt-5 flex flex-col gap-2">
            {donutData.map((d) => (
              <li key={d.label} className="flex items-center gap-2.5 text-sm">
                <span aria-hidden="true" className="w-2.5 h-2.5 rounded-sm shrink-0" style={{ backgroundColor: d.color }} />
        ...
        <section aria-label="Recent uploads">
          <div className="flex items-baseline justify-between">
            ...
            <Link href="/admin/library" className="text-xs text-liol-subtext hover:text-liol-text duration-200">
              View all →
            </Link>
          </div>
          {recentUploads.length === 0 ? (
            ...
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-2.5">
              {recentUploads.map((photo, i) => (
                <figure key={photo.id} className="relative aspect-square overflow-hidden rounded-lg">
                  <Image
                    src={photo.src}
                    alt={photo.alt_text}
                    fill
